// =====================================================================
// BABASITARAM PRO -- background.js (FIREFOX BUILD)
// Bundle this file ONLY with manifest-firefox.json. This is identical to
// the Chrome build except the per-tab match-count lookup no longer calls
// an API Firefox doesn't implement -- it relies solely on the persistent
// trackerDomainStats counter (built from real-time messages from
// content.js), which is exactly what the Chrome build falls back to
// anyway once that API's 5-minute buffer rolls over. No functionality is
// lost: the "matched rules for this tab" number is populated the same
// way, just without a code path that would otherwise trip AMO's
// UNSUPPORTED_API warning regardless of any runtime guard around it.
// =====================================================================
﻿﻿// ═══════════════════════════════════════════════════════════════════
...
async function handleMessage(msg, sender) {
  switch (msg.action) {
    ...
    // -- Per-tab match count for current tab (persistent-stat based; this
    // build intentionally does not call any per-tab live-match API) --
    case "getDNRCount": {
